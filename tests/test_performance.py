...
class TestPerformanceBenchmarks:
    ...
    def test_insert_performance(self, test_client):
        ...
        try:
            ...
            for batch_size in batch_sizes:
                ...
                # Generate test data
                test_data = TestDataGenerator.generate_complex_data(batch_size, embedding_dim=128)
                ...
                print(f"Batch size {batch_size}: {insert_time:.2f}s, {throughput:.2f} records/sec")
    ...
    def test_query_performance(self, test_client):
        ...
        try:
            ...
            test_data = TestDataGenerator.generate_complex_data(5000, embedding_dim=64)

            collection = collection_manager.create_collection_with_data(collection_name, schema, test_data)
    ...
    def test_comparison_performance(self, test_client):
        ...
        try:
            ...
            # Create collection with data
            test_data = TestDataGenerator.generate_complex_data(2000, embedding_dim=32)
            ...
            start_time = time.time()
            comparison_result = test_client.entity_compare(collection, batch_size=500, full_scan=True)
            end_time = time.time()
...
class TestStressTests:
    ...
    def test_concurrent_operations_stress(self, test_client):
        ...
        try:
            ...
            # Create collection
            collection = collection_manager.create_collection_with_data(collection_name, schema, [])

            def insert_batch(batch_id: int, batch_size: int = 100):
                """Insert a batch of data."""
                data = TestDataGenerator.generate_complex_data(
                    batch_size, start_id=batch_id * batch_size, embedding_dim=16
                )
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                # Submit insert tasks
                insert_futures = [executor.submit(insert_batch, i) for i in range(5)]
                ...
                # Submit query tasks
                query_futures = [executor.submit(query_data, i) for i in range(10)]
    ...
    def test_large_dataset_stress(self, test_client):
        ...
        try:
            ...
            for i in range(0, total_records, batch_size):
                batch_data = TestDataGenerator.generate_complex_data(batch_size, start_id=i, embedding_dim=64)
            ...
            # Verify final count
            final_count = test_client.count(collection)
            print(f"Final count - Milvus: {final_count['milvus_count']}, PostgreSQL: {final_count['pg_count']}")
