...
def main():
    ...
    logging.info(f"Starting {NUM_INSERTS} insert operations in batches of {BATCH_SIZE}...")
    insert_start_id = 0
    ...
    logging.info(f"Starting {NUM_DELETES} delete operations in batches of {BATCH_SIZE}...")
    ...
    # 1M new inserts
    logging.info(f"Starting {NUM_UPSERTS} upsert operations in batches of {BATCH_SIZE}...")
    ...
    try:
        ...
        res_query = milvus_client.query(collection_name, expr=query_expr, output_fields=["id", "name", "age"])
        ...
        logging.warning("Actual count query is commented out as it can be slow. You can enable it if needed.")
    ...
    logging.warning("Export is commented out as it can produce very large output for 5M records.")
