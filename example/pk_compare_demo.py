...
def demonstrate_pk_operations(client: MilvusPGClient, collection_name: str):
    ...
    result_with_pk_check = client.entity_compare(collection_name, compare_pks_first=True, full_scan=False)
    logger.info(f"启用主键比较的 entity_compare 结果: {result_with_pk_check}")

    # 不启用主键比较的 entity_compare（原有行为）
    result_without_pk_check = client.entity_compare(collection_name, compare_pks_first=False, full_scan=False)
